...
| `Штрихкод` | `barcode` | в metadata каталога нет — читаем, если появится |
| `СреднийВес` | `weight` | |
| `Модель`, `Вставка`, `Комментарий`, `ЮС_ВнешнийВид`, `КС_КатегорияВставок` | `characteristics` | склейка текста |

**Фильтр переноса (Excel «Перенос в ИИ»):** при sync номенклатуры оставляем только направления своей базы — `asil`: ИМПЕРИАЛ / ИМПЕРИАЛ KZ; `miamor`: БЕЛЛА / МиАмор. Пустое направление отбрасывается. Контрагенты — только дерево папки **«Покупатели»** (если папки нет, пишем warning и синкаем всех).
...
Имена баз задаются в админке. У синхронизированных строк остаётся технический `source_id`.
